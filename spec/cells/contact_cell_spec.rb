...
describe ContactCell do
  ...
  context "cell rendering" do
    ...
    it { is_expected.to have_selector("span#total-time", :text => QueryObjectMock.new.total_time(nil)/60) }
    it { is_expected.to have_selector("span#unsupervised-contact-time", :text => QueryObjectMock.new.unsupervised_contact_time(nil)/60) }
    it { is_expected.to have_selector("span#supervision-message", :text => SupervisionTimeMock.from_minutes(nil)) }

    context "rendering adult time" do
      it { is_expected.to have_selector("legend", :text => "Adult") }
    end
    ...
    context "rendering child time" do
      ...
      it { is_expected.to have_selector("legend", :text => "Child") }
    end
  end
